/// The WHATWG `URL` / `URLSearchParams` subset for the compat layer.
///
/// Node-faithful behaviors (the commonly scripted ones):
///
/// * `new URL(url[, base])` — throws `TypeError('Invalid URL')` on garbage;
///   relative resolution against `base` (scheme-relative, absolute path,
///   relative path, query-only, hash-only);
/// * special schemes (`http`, `https`, `ws`, `wss`, `ftp`, `file`) get
///   default-port elision in `href`, an empty path normalized to `/`, and
///   `origin` `scheme://host[:port]`; `file` has `origin` `'null'`;
/// * non-special schemes get `origin` `'null'`, an empty pathname when no
///   path follows the authority, and no default-port elision;
/// * all accessors with setters (`href`, `protocol`, `username`,
///   `password`, `host`, `hostname`, `port`, `pathname`, `search`, `hash`,
///   `searchParams` live-bound both ways with `search`);
/// * `URLSearchParams` with the form-urlencoded codec (`+` for spaces on
///   serialize, `%xx` round-trip), `append/delete/get/getAll/has/set/sort/
///   forEach/entries/keys/values/toString` and the `size` getter;
/// * `URL.canParse(url[, base])`, `URL.parse(url[, base])` (null instead
///   of throwing), `createObjectURL`/`revokeObjectURL` call-time stubs.
///
/// Not provided: full WHATWG state-machine percent-normalization of exotic
/// code points, IDNA/punycode host conversion, IPv6 scope IDs, and `url`
/// legacy API (`urlObject.href` style). `require('url')` exposes `URL`,
/// `URLSearchParams`, `pathToFileURL`, `fileURLToPath` (posix).
const String nodeCompatUrlPrelude = r'''
(function () {
    var SPECIAL = {
