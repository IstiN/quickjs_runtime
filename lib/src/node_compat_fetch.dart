/// The sync `fetch` / `Headers` / `Response` surface for the compat layer,
/// evaluated only when [NodeCompatConfig.httpFetch] is set.
///
/// Node-faithful where the sync model allows it:
///
/// * `fetch(input[, init])` — `input` is a string, `URL`, or Request-like
///   (`{url, method?, headers?}`); `init` carries `method`, `headers`,
///   `body`; `init.signal` is accepted and ignored (nothing to abort in a
///   synchronous runtime); GET/HEAD with a body throws `TypeError` like
///   Node; network failures surface as `TypeError: fetch failed` with the
///   transport message in `error.cause`.
/// * `Headers` — case-insensitive `get`/`has`/`set`/`append`/`delete`/
///   `forEach`/`entries`/`keys`/`values`, accepts records and pair arrays.
/// * `Response` — `ok` (200–299), `status`, `statusText`, `url`,
///   `redirected` (always false), `headers`, one-shot body via `text()` /
///   `json()` / `arrayBuffer()` / `bytes()` with the Node `bodyUsed`
///   guard (`TypeError` on the second read).
///
/// Deliberate deviation, documented: the body accessors return plain
/// values instead of Promises (no event loop). This is **await-
/// compatible** — `await response.text()` works unchanged, because
/// `await` passes non-thenable values through — while remaining usable
/// in fully synchronous scripts.
///
/// Not provided: streams (`res.body` is `undefined`), `blob()` /
/// `formData()`, `Response.json()` static, request/progress events.
const String nodeCompatFetchPrelude = r'''
(function () {
