...
///
/// * `Buffer` **is** a `Uint8Array` subclass (`buf instanceof Uint8Array`,
///   `Object.prototype.toString.call(buf)` answers `'[object Uint8Array]'`,
///   `.length`, indexing, iteration all behave like a typed array);
/// * `Buffer.from(string[, encoding])` with `utf8`, `utf16le` (+`ucs2`),
///   `latin1` (+`binary`), `ascii`, `hex`, `base64`, `base64url`;
/// * `Buffer.from(buffer)` copies, `Buffer.from(arrayBuffer)` creates a
///   view, `Buffer.from({type: 'Buffer', data: [...]})` revives JSON form;
/// * `Buffer.alloc` / `Buffer.allocUnsafe` / `Buffer.concat` /
///   `Buffer.byteLength` / `Buffer.isBuffer` / `Buffer.compare`;
/// * instance `toString([encoding[, start[, end]]])`, `write`, `fill`,
///   `copy`, `equals`, `compare`, `indexOf`/`lastIndexOf`/`includes`,
///   `subarray`/`slice`, `toJSON`, and the little-endian/big-endian
///   `read*/write*` primitive accessors over a `DataView`.
///
